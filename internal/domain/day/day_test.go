...
func TestUpdateDay_InvalidSlug(t *testing.T) {
	day := &Day{ID: "test-id", Name: "Day A", Slug: "day-a"}
	originalSlug := day.Slug

	invalidSlug := "Invalid Slug"
	input := UpdateDayInput{Slug: &invalidSlug}

	result := UpdateDay(day, input)

	if result.Valid {
		t.Error("UpdateDay with invalid slug returned valid result")
	}
	if day.Slug != originalSlug {
		t.Errorf("day.Slug was changed despite validation failure")
	}
}

func TestDay_Validate_InvalidMetadata(t *testing.T) {
	day := &Day{
		ID:   "test-id",
		Name: "Day A",
		Slug: "day-a",
		Metadata: map[string]interface{}{
			// We need to construct something that fails to marshal
			// Since normal maps marshal fine, we test valid metadata passes
		},
	}
...
func TestDay_Validate_MultipleErrors(t *testing.T) {
	day := &Day{
		ID:   "test-id",
		Name: "",          // Invalid
	...
	if result.Valid {
		t.Error("Validate returned valid result for day with multiple errors")
	}
	if len(result.Errors) < 2 {
		t.Errorf("Expected at least 2 errors, got %d", len(result.Errors))
	}
...
func TestCreateDay_InvalidMetadata(t *testing.T) {
	// Creating metadata that fails JSON marshaling is tricky with normal types
	// Testing with a channel won't work as map values must be interface{}
	// However, we can test that valid metadata works
	input := CreateDayInput{
		Name:     "Day A",
	...
	if !result.Valid {
		t.Errorf("CreateDay with valid metadata returned invalid result: %v", result.Errors)
	}
	if day.Metadata == nil {
		t.Error("day.Metadata should not be nil")
	}
}

func TestUpdateDay_InvalidMetadata(t *testing.T) {
	day := &Day{ID: "test-id", Name: "Day A", Slug: "day-a"}

	// Test that valid metadata works
	metadata := map[string]interface{}{"key": "value"}
	input := UpdateDayInput{Metadata: metadata}
	...
	if !result.Valid {
		t.Errorf("UpdateDay with valid metadata returned invalid result: %v", result.Errors)
	}
}

func TestCreateDay_EmptySlugAutoGenerated(t *testing.T) {
	input := CreateDayInput{
	...
	if day.Slug != "heavy-squat-day" {
		t.Errorf("day.Slug = %q, want %q (auto-generated)", day.Slug, "heavy-squat-day")
	}
}
